...
impl IconRenderer {
    ...
    pub fn render_weather_icon<const SIZE: usize>(
        ...
        match condition {
            ...
            WeatherCondition::LightRain | WeatherCondition::ModerateRain | WeatherCondition::HeavyRain => {
    ...
    /// 渲染太阳
    fn draw_sun<const SIZE: usize>(&self, framebuffer: &mut Framebuffer<SIZE>, x: u16, y: u16) -> SystemResult<()> {
        // 太阳本体 - 绘制一个矩形
    ...
    /// 渲染云
    fn draw_cloud<const SIZE: usize>(&self, framebuffer: &mut Framebuffer<SIZE>, x: u16, y: u16) -> SystemResult<()> {
        // 绘制简单的云形 - 矩形
    ...
    /// 渲染雨
    fn draw_rain<const SIZE: usize>(&self, framebuffer: &mut Framebuffer<SIZE>, x: u16, y: u16) -> SystemResult<()> {
        // 绘制雨滴 - 小矩形
    ...
    /// 渲染闪电
    fn draw_lightning<const SIZE: usize>(&self, framebuffer: &mut Framebuffer<SIZE>, x: u16, y: u16) -> SystemResult<()> {
        // 绘制闪电 - 矩形
    ...
    /// 渲染雪
    fn draw_snow<const SIZE: usize>(&self, framebuffer: &mut Framebuffer<SIZE>, x: u16, y: u16) -> SystemResult<()> {
        // 绘制雪花 - 小点
    ...
    /// 渲染雾
    fn draw_fog<const SIZE: usize>(&self, framebuffer: &mut Framebuffer<SIZE>, x: u16, y: u16) -> SystemResult<()> {
        // 绘制雾 - 矩形
