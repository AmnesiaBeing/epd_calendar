...
use crate::core::system::api::SystemApi;
...
pub struct SourceMeta {
    ...
    pub id: DataSourceId,
    /// 数据源实例
    pub instance: &'static Mutex<dyn DataSource>,
    /// 刷新间隔（秒）
...
impl DataSourceScheduler {
    ...
    pub fn register_source(
        &mut self,
        id: DataSourceId,
        instance: &'static Mutex<dyn DataSource>,
        interval_secs: u32,
    ) -> Result<()> {
        // 检查数据源是否已注册
        if self.sources.iter().any(|s| s.id == id) {
            warn!("DataSource {:?} already registered", id);
            return Ok(());
        }
        ...
        info!("Registered DataSource {:?}, interval: {}s, new min interval: {}s", 
              id, interval_secs, self.min_interval);
    ...
    pub async fn refresh_source(
        ...
        event_chan: &Channel<DataSourceEvent, 8>,
    ) -> Result<()> {
        // 查找数据源
    ...
    pub async fn refresh_all(
        ...
        for source_meta in self.sources.iter_mut() {
            ...
            if source.refresh(system_api).await.is_ok() {
                source_meta.last_refresh_tick = now;
                event_chan.send(DataSourceEvent::Updated(source_meta.id)).await.ok();
                debug!("[{}] Refreshed in refresh_all", source_meta.id);
            }
        }
...
pub async fn generic_scheduler_task(
    scheduler: &'static Mutex<DataSourceScheduler>,
    system_api: &'static dyn SystemApi,
    event_chan: &'static Channel<DataSourceEvent, 8>,
) {
    info!("Starting DataSource scheduler task");
    
    // 初始化：获取最小刷新间隔
    let min_interval = scheduler.lock().await.get_min_interval();
    let mut ticker = Ticker::every(Duration::from_secs(min_interval));
    
    info!("Scheduler ticker set to {} seconds", min_interval);
    
    loop {
        ticker.next().await; // 低功耗休眠，到点唤醒
        
        debug!("Scheduler tick - checking data sources");
        ...
        for source_meta in guard.sources.iter_mut() {
            // 判断是否达到刷新时间
            if now - source_meta.last_refresh_tick >= source_meta.interval_secs * 1000 { // 转换为毫秒
                debug!("[{}] Ready for refresh (now: {}, last: {}, interval: {})
", 
                      source_meta.id, now, source_meta.last_refresh_tick, source_meta.interval_secs);
                ...
                match source.refresh(system_api).await {
                    Ok(_) => {
                        ...
                        event_chan.send(DataSourceEvent::Updated(source_meta.id)).await.ok();
                        debug!("[{}] Refreshed successfully", source_meta.id);
                    },
                    Err(e) => {
                        warn!("[{}] Refresh failed: {:?}", source_meta.id, e);
                        event_chan.send(DataSourceEvent::Failed(source_meta.id, e)).await.ok();
                    }
                }
            }
        }
