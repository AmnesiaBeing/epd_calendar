...
use crate::kernel::data::types::{DataSourceId, DynamicValue, FieldMeta, FieldType};
...
impl DataSource for WeatherDataSource {
    ...
    async fn refresh(
        ...
        // 从配置数据源获取location_id
        let location_id = match system_api
        ...
        match system_api.http_get(&url).await {
            Ok(response) => {
                ...
                self.cache.set_field(
                    String::from("weather.temperature"),
                    DynamicValue::Float(weather_data.0 as f64),
                )?;
                self.cache.set_field(
                    String::from("weather.humidity"),
                    DynamicValue::Integer(weather_data.1 as i64),
                )?;
                self.cache.set_field(
                    String::from("weather.condition"),
                    DynamicValue::String(String::from(weather_data.2)),
                )?;
            ...
            Err(e) => {
                ...
                self.cache.set_field(
                    String::from("weather.temperature"),
                    DynamicValue::Float(temperature as f64),
                )?;
                self.cache.set_field(
                    String::from("weather.humidity"),
                    DynamicValue::Integer(humidity as i64),
                )?;
                self.cache.set_field(
                    String::from("weather.condition"),
                    DynamicValue::String(String::from("本地数据")),
                )?;
            }
        }
