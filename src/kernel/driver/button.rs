...
impl EspButtonDriver {
    /// 创建新的按键驱动实例
    pub fn new(peripherals: &Peripherals, config: ButtonConfig) -> Result<Self> {
        log::info!("Initializing ESP button driver on GPIO9 with config: {:?}", config);
...
impl ButtonDriver for MockButtonDriver {
    async fn get_event(&mut self) -> Result<ButtonEvent> {
        // 模拟按键事件（实际无硬件）
        esp_rtos::time::sleep(Rate::from_secs(10)).await;
