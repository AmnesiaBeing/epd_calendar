...
use crate::common::display::TimeData;
...
impl TimeService {
    pub fn new(time_source: DefaultTimeSource) -> Self {
        Self { time_source }
    }

    pub async fn update_time_by_sntp(&mut self, ntp_source: &mut SntpSource) -> Result<()> {
        // TODO: 增加平滑修改时间的逻辑（需要修改TimeSource）
        let sntp_time = ntp_source.sync_time().await?;
