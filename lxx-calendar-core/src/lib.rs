...
use lxxcc::{SystemEvent, SystemResult};
use lxxcc::types::async_types::{Channel, ChannelReceiver};
...
pub async fn core_main<M: lxxcc::types::async_types::AsyncRawMutex + Default + 'static>(
    spawner: Spawner
) -> SystemResult<()> {
    // 注意：这里需要平台实现提供具体的platform实例
    // 目前暂时使用空实现，需要根据实际平台实现进行调整
    lxxcc::info!("lxx-calendar starting...");

    // 创建系统事件通道并确保其具有'static生命周期
    let channel = Box::new(Channel::<M, SystemEvent, 32>::new());
    ...
    lxxcc::info!("Main task started");

    loop {
        match state_manager.wait_for_event().await {
            Ok(event) => {
                lxxcc::info!("Received event: {:?}", event);
                if let Err(e) = handle_event(&mut state_manager, event).await {
                    lxxcc::error!("Failed to handle event: {:?}", e);
                }
            }
            Err(e) => {
                lxxcc::error!("Failed to wait for event: {:?}", e);
            }
        }
    }
}

async fn handle_event<M: lxxcc::types::async_types::AsyncRawMutex>(
